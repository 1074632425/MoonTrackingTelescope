#Project: Moon Tracking Telescope
# 11/1/2019

# High Level Description:
# MoonTracking.py performs efficent object recognition of white 
# objects viewed on camera using multi-processing. It then
# extract the center position of the largest white object(Moon) and 
...
# Press "e" to take a screen shot.
# Press "m" to enter/exit calibration mode.



# Low Level Description:
# MoonTracking.py would start four processes: one master process 
...
pi_hw.hardware_PWM(motorPinR, 50, 51900) #50 Hz Freq.
pi_hw.hardware_PWM(motorPinT, 50, 92400) #50 Hz Freq. 
print "CALIBRATE CAMERA in 3 sec"
time.sleep(3)
currentDirectionR = 51900
currentDirectionT = 92400
...
def grab_frame_display(run_flag, send_frame_queue, receive_contour_queue, p_start_turn, p_end_turn, p_start_lock, p_end_lock):
	...
	while (run_flag.value):
		...
		# 4. Check the sum of white pixels, only try to send over for processing if greater than threshold
		...
		if (calibrationNow):
			pi_hw.hardware_PWM(motorPinR, 50, currentDirectionR) #50 Hz Freq.
			pi_hw.hardware_PWM(motorPinT, 50, currentDirectionT) #50 Hz Freq.
		else: 
			# Only put frame in queue if it has past 30ms and exceeds color threshold and there are fewer than 4 frames in queue
			...
			if ((not receive_contour_queue.empty())):
				...
				# Extract the contour of the largest white object
				if (len(contours)>0):
				...
				# Compute center position of the largest white object
				if (M["m00"] != 0):
					cX = int(M["m10"] / M["m00"])
					cY = int(M["m01"] / M["m00"])
		...
		elif k == ord('a'): # Press a to turn left	
			rotate5(-1,1)
			print(currentDirectionR)

		elif k == ord('d'): # Press d to turn right
			rotate5(1,1)
			print(currentDirectionR)

		elif k == ord('w'): # Press w to turn up
			tilt5(1,1)
			print(currentDirectionT)

		elif k == ord('s'): # Press s to turn down
			tilt5(-1,1)
			print(currentDirectionT)

		elif k == ord('e'): # Press e to take a screenshot
			cv2.imwrite('/home/pi/Moon/ScreenShot/'+str(u)+'.jpg',frame)
			print("take a screenshot as "+str(u)+".jpg")
			u+=1
		elif k == ord('m'): # Press m to enter/exit calibration mode 
...
def process_frame_1(run_flag, send_frame_queue, receive_contour_queue, p_start_turn, p_end_turn, p_start_lock, p_end_lock):
	while (run_flag.value):
		...
		if ((not send_frame_queue.empty()) and (p_start_turn.value == 1)):
			...
			# 2. Extract contour
			image,contours,h=cv2.findContours(maskClose.copy(),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
